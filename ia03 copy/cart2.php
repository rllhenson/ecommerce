  ...
  foreach ($_SESSION["cart_array"] as $each_item) {
    ...
    if($count > 0){
      while ($row=$result->fetch_assoc()) {
		...
		$cartOutput .= "<tr>";
		$cartOutput .= '<td><a href="product.php?id=' . $item_id . '">' . $product_name . '</a><br /><img src="img/'. $img .'" alt="' . $product_name. '" width="150" height="150" border="1" /></td>';
		$cartOutput .= '<td>' . $details . '</td>';
		$cartOutput .= '<td>$' . $price . '</td>';
		$cartOutput .= '<td><form action="cart.php" method="post">
		
		<!-- Edit Cart Quantity Input Box Size --> 
		<input name="quantity" id="itemquantity" type="text" value="' . $each_item['quantity'] . '" maxlength="2" />
		<input id="changebutton" name="adjustBtn' . $item_id . '" type="submit" value="change" />
